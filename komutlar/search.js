...
module.exports = {
  ...
  var songInfo = video
   ...
   const queueConstruct = {
    ...
    if (!song){
      if (!online.afk) {
        sendError("Sırada şarkı olmadığını düşündüğüm için ses kanalından ayrılıyorum. Botu beğendiyseniz 7/24 ses kanalında kalın `n!Afk`", message.channel)
...
stream.on('error', function(er)  {
      if (er) {
        ...
        queue.songs.shift();
        play(queue.songs[0]);
  	  return sendError(`Beklenmeyen bir hata oluştu. \ NOlası tip \`${er}\``, message.channel)
